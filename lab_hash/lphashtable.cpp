...
void LPHashTable<K, V>::remove(K const& key)
{
     ...
     * @todo: implement this function
     */
    unsigned int current = hashes::hash(key, size);
    while (table[current] != NULL && table[current]->first != key) {
        current = (current + 1) % size;
    }
    if (table[current] == NULL) return;
    else if (table[current]->first == key) {
...
int LPHashTable<K, V>::findIndex(const K& key) const
{
    ...
    while (should_probe[current]) {
        if (table[current] != NULL) {
            if (table[current]->first == key) return current;
        }
        current = (current + 1) % size;
    }
    return -1;
...
void LPHashTable<K, V>::resizeTable()
{
    ...
    for (size_t slot = 0; slot < size; slot++) {
        if (table[slot] != NULL) {
            unsigned int current = hashes::hash(table[slot]->first, new_size);
            while (new_table[current] != NULL) {
                current = (current + 1) % new_size;
            }
            new_table[current] = table[slot];
