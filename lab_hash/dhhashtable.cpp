...
int DHHashTable<K, V>::findIndex(const K& key) const
{
     ...
     * @todo Implement this function
     */
    size_t first= hashes::hash(key, size);
    size_t second = hashes::secondary_hash(key, size);
    size_t i = 0;
    size_t idx = first;
    do{
...
void DHHashTable<K, V>::resizeTable()
{
    ...
    for (size_t slot = 0; slot < size; slot++) {
        if (table[slot] != NULL) {
            size_t hashFirst= hashes::hash(table[slot]->first, newSize);
            size_t hashSecond = hashes::secondary_hash(table[slot]->first, newSize);
            size_t i = 0;
            size_t idx = hashFirst; 
            while (temp[idx] != NULL)
            {
                ++i;
                idx = (hashFirst+ hashSecond*i) % newSize;
            }
            temp[idx] = table[slot];
            should_probe[idx] = true;
        }
