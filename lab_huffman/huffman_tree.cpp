...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() > 1) {
        ...
        TreeNode* smallestNext = removeSmallest(singleQueue, mergeQueue);

        int freq1 = (smallest->freq).getFrequency();
        int freq2 = (smallestNext->freq).getFrequency();
        TreeNode* merged = new TreeNode(freq1 + freq2);
        merged->left = smallest;
        merged->right = smallestNext;
