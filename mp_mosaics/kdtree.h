...
class KDTree
{
  ...
  public:
    ...
    void buildTree(vector<Point<Dim>>& newPoints, int left, int right, int dimension, KDTreeNode*& subroot);

    int split(vector<Point<Dim>>& list, int left, int right, size_t middle, int dimension);

    Point<Dim> quickAlg(vector<Point<Dim>>& list, int left, int right, size_t k, int dimension);
    ...
    void findNearHelper(KDTreeNode* curr_node, Point<Dim> &currentBest, Point<Dim> &query, int dimension) const;
    //please dont reach this function (wellThen)
    void wellThen();
