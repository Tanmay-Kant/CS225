...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    string newone = word1;
    string newtwo = word2;
    std::transform(newone.begin(), newone.end(), newone.begin(), ::toupper);
    std::transform(newtwo.begin(), newtwo.end(), newtwo.begin(), ::toupper);
    auto one = dict.find(newone);
	auto two = dict.find(newtwo);
    if(one == dict.end() || two == dict.end()){
      return false;
    }
